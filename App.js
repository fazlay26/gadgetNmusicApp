...
import { StyleSheet, Text, View, TouchableOpacity, FlatList, ActivityIndicator, BackHandler } from 'react-native';
...
export default function App() {
  const [url, setUrl] = useState('https://gadgetnmusic.com/');
  ...
  registerNNPushToken(12974, 'FhdECwzk3aqDPL0GKwliMG');
  ...
  useEffect(() => {
    async function getUnreadNots() {
      let unreadCount = await getUnreadNotificationInboxCount(12974, 'FhdECwzk3aqDPL0GKwliMG');
  ...
  useEffect(() => {
    console.log('Registering back button event listener');
    const backAction = () => {
      ...
      return false; // Let the app exit if there is no web history
    };

    const backHandler = BackHandler.addEventListener('hardwareBackPress', backAction);

    return () => {
      console.log('Removing back button event listener');
      backHandler.remove();
    };
  }, []);
